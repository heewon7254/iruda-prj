$(document).ready(function(){
    ...
    gnb.on('click', (e) => {
        $(e.currentTarget).toggleClass('on').siblings('.lnb').stop().slideToggle('fast');
    });
    ...
    gnb.on('focus', (e) => {
        if (isTab && !$(e.currentTarget).hasClass('on')) {
            $(e.currentTarget).addClass('on').siblings('.lnb').stop().slideDown('fast');
        }
    });
    ...
    $layerActiveTarget.on('focus', function () {
        ...
        if (targetsInParent === 1 && layersInParent === 1) {
            ...
            $parentLayerArea.find(`.layer-box[data-layer="${targetLayer}"]`).addClass('show');
        }
    });
    ...
    // 인기데이터 셋 현황 슬라이드
    function initSlide(containerSelector, itemSelector, itemCount, itemHeight, intervalTime) {
    ...
    initSlide('.rank-slide', '.desc', 5, 36, 3000);
    initSlide('.title-slide', '.text', 5, 19, 3000);
    ...
    $('.filter-con-title').on({
        click(e) {
            ...
            $(e.currentTarget).siblings('.filter-content-area').stop(true, true).slideToggle('fast');
        },
        focus(e) {
            let $target = $(e.currentTarget).siblings('.filter-content-area');
            if (!$target.is(':visible')) $target.stop(true, true).slideDown('fast');
        }
    });

    // 필터 체크박스
    const filterCheckbox = $('.filter-content .checkbox-group input[type="checkbox"]');
    filterCheckbox.click(e =>$(e.currentTarget).parents('.checkbox-group').toggleClass('active'))
